...
## 6.4 나머지 감사 항목 (2026-09-13)

앞선 감사에서 남아 있던 항목을 차례로 확인하고 고쳤다.

- **패키징본에서도 `ELECTRON_RENDERER_URL` 을 읽었다.** 이 변수를 넣으면 CSP 가 완화되고 DevTools 가 켜지며, 프리로드 브리지가 붙은 채로 원격 페이지가 로드된다. `app.isPackaged` 로 개발 모드에서만 읽는다.
- **`will-navigate` 가 프로덕션에서 모든 `file://` 을 허용했다.** 창에 .html 파일을 떨어뜨리면 그 페이지로 이동하고, 그 페이지가 브리지를 물려받는다. 이 창이 보여 주기로 한 그 한 페이지만 허용한다.
- **세션 허가 키가 명령을 400자로 잘랐다.** 앞 400자가 같은 다른 명령이 무승인 실행될 수 있었다. 400자 뒤에 전체 명령의 다이제스트를 붙인다.
- **승인 단축키(Y/S/N)가 '가장 최근' 요청을 답했다.** 요청이 둘 이상 열려 있으면 읽고 있는 카드가 아닐 수 있다. 하나뿐일 때만 동작하고, 아니면 클릭을 요구한다.
- **셸로 읽는 민감 파일이 승인 판정에 반영되지 않았다.** `fs/read_text_file` 로 `.env` 를 읽으면 걸리는데 `cat .env` 는 통과했다. 명령 인자의 경로도 민감도 검사를 거친다.
- **비밀 마스킹.** JSON 표기(`"API_TOKEN": "…"`)와 소문자 키를 놓쳤고, github_pat_·glpat-·Stripe·Google OAuth·Slack 웹훅·Anthropic 키 형식이 규칙에 없었다. 또 diff 미리보기가 자른 뒤에 마스킹해서, 경계에 걸친 PEM 키가 END 마커를 잃고 평문으로 남았다 — 마스킹을 먼저 한다.
- **세션 제목**이 사용자의 첫 프롬프트 원문 그대로 `metadata.json` 에 들어갔다. 마스킹을 거친다.
- **`shell.openPath`** 가 작업공간 안이면 무엇이든 OS 기본 핸들러로 열었다. `.app`·`.command`·`.sh` 류는 열지 않고 Finder 를 안내한다.
- **`session/prompt` 에 180초 타임아웃**이 걸려 있었다. 사람이 승인 카드를 3분 넘게 보고 있으면 턴이 '실패'로 표시되는데 CLI 는 계속 돈다. 턴에는 클라이언트 타임아웃을 두지 않는다 (중단은 명시적이고, 자식이 죽으면 종료 처리가 잡는다).
- **고아 프로세스 정리**가 `grok` + `agent` 만 확인해서, PID 가 재사용되면 사용자가 터미널에서 직접 띄운 grok 을 죽일 수 있었다. 앱이 띄우는 형태(`agent stdio`)까지 확인한다.
- **ChangeTracker 가 크기 제한 없이 파일을 읽었다.** 거대한 파일을 쓰면 메인 프로세스가 통째로 들고 있었다. 인라인 한도를 넘으면 스냅샷을 만들지 않는다.
- **진단 로그에 회전이 없었다.** 5MB 를 넘으면 한 세대만 남기고 새로 시작한다.
- **Electron 43.4.0 → 43.7.0.** 같은 메이저의 Chromium 보안 롤업 5개가 빠져 있었다 (렌더러는 에이전트가 만든 마크다운을 그린다).
