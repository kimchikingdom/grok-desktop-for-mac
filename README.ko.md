...
이슈와 풀 리퀘스트를 환영합니다. PR 전에 `pnpm verify`(린트·타입체크·단위·보안 테스트)와
`pnpm test:e2e`를 돌려 주세요. 보안과 관련된 변경에는 테스트를 함께 주시면 좋습니다 —
승인 게이트, 경로 봉쇄, 비밀 마스킹은 `packages/security`와 `tests/security`에 스위트가 있습니다.

## 라이선스

[MIT](LICENSE). 쓰고, 고치고, 배포해도 됩니다. 저작권 표기만 남겨 주세요.

라이선스는 이 앱의 소스에만 적용됩니다. "Grok"·"xAI" 이름이나 마크에 대한 권리는 주지 않으므로,
포크를 공개한다면 이름과 아이콘은 직접 지어 쓰세요. 이 앱은 별도로 설치한 Grok Build CLI를
실행할 뿐, CLI나 xAI 서비스를 재배포하지 않습니다. CLI와 xAI 서비스 이용은 각자의 약관을 따릅니다.
